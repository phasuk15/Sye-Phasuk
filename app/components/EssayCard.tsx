interface EssayCardProps {
  ...
  title: string;
  description: string;
  fileLabel?: string;   // "essay.txt" or "essay.pdf"
}

const EssayCard = ({ date, title, description, fileLabel = "essay.txt" }: EssayCardProps) => {
  return (
    <div className="p-5 border-4 border-raspberry bg-white shadow-[6px_6px_0px_#412722] hover:shadow-[8px_8px_0px_#AE5969] transition-all duration-200 cursor-pointer font-pixelify max-w-2xl">
      
      <div className="bg-raspberry text-light-pink px-2 py-1 flex items-center justify-between text-xs mb-3">
        <span className="font-bold">{fileLabel}</span>
